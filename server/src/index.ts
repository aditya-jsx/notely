...
//     } catch (e) {
//         console.error("Error during Google verification", e);
//         return res.status(500).json({ msg: "An unexpected error occurred during Google sign-in" });
//     }
// });
...
app.post("/api/v1/auth/google/verify", async (req, res) => {
    ...
    // This check confirms to TypeScript that GOOGLE_CLIENT_ID is a string
    if (!GOOGLE_CLIENT_ID) {
    ...
    try {
        const ticket = await googleClient.verifyIdToken({
            idToken: credential,
            audience: GOOGLE_CLIENT_ID, // This is now type-safe
        });

        const payload = ticket.getPayload();

        // This check solves the 'getPayload' error and ensures the payload is valid
        if (!payload || !payload.email || !payload.name) {
        ...
        console.error("Error during Google verification", e);
        return res.status(500).json({ msg: "Google sign-in failed. The token may be invalid." });
    }
});

// --- Existing manual auth routes ---
app.post("/api/v1/signup", async (req, res)=>{
    const requiredBody = z.object({
        name: z.string().min(1, "Name is required"),
        email: z.string().email(),
